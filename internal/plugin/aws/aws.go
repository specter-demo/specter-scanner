...
func (p *Plugin) scanBedrock(ctx context.Context) ([]types.CanonicalAgentRecord, []types.FindingRecord, error) {
	...
	for {
		...
		for _, summary := range out.AgentSummaries {
			...
			// platform surfaces as a suggested intent for the operator to confirm —
			...
			//
			// Description text is platform-agnostic — this finding is generated by
			// the AWS plugin directly, which has no visibility into NoPlatform/standalone
			...
			findings = append(findings, types.FindingRecord{
				...
				Description: fmt.Sprintf(
					"Bedrock agent %s has no declared intent. Add a specter:intent tag to the Bedrock agent resource.",
					name,
				),
				EvidenceJSON: evidence,
				DiscoveredAt: now,
